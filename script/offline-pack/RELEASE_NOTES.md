...
1. 把 **所有** `mangajanai-win.7z.00N` 分卷和 `合并分卷并校验.bat` 下载到**同一个文件夹**；
2. 双击 `合并分卷并校验.bat` → 得到单一的 `mangajanai-win.7z`（约 2.x GB）；
...
> 合并必须是**字节拼接**（`合并分卷并校验.bat` 做的就是这件事），不能用 7-Zip 直接解压 `.001`
